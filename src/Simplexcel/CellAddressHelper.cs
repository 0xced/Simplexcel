...
{
    private static readonly Regex CellAddressRegex = new("(?<Column>[a-z]+)(?<Row>[0-9]+)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    internal static void ReferenceToColRow(string reference, out int row, out int column)
    {
        ...
        var parts = CellAddressRegex.Match(reference);
        if (parts.Success)
        {
    ...
    internal static IList<string> DetermineRanges(ICollection<CellAddress> cellAddresses)
    {
        ...
        foreach (var ca in cellAddresses ?? [])
        {
